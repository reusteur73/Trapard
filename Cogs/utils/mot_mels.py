...
import sys
import string
from .path import LISTE_MOTS

# x = random.choices(data, )
...
class Grid(object):
    ...
    def __init__(self, options):
        ...
        words = open(LISTE_MOTS).read().splitlines()
        while True:

            selected_words = random.choices(words, k=12)
            selected_words.sort()
            if len(set(selected_words)) == 12:
                # Les 12 mots sont uniques et différents
                print("Les mots sélectionnés sont tous uniques et différents.")
                break
            else:
                # Il y a des mots en double ou des répétitions
                print("Certains mots sélectionnés ne sont pas uniques ou différents.")
        words = selected_words
        self.wanted_words = words
        words.sort()
    ...
    def key(self, fancy=True):
        ...
        for i in range(int(column_height)):
            ...
            for j in range(int(number_of_columns)):
                ...
                x = i + j * column_height
                if x < len(words):
                    row += words[int(x)].ljust(column_width)
...
# if __name__ == '__main__':
    ...
    # print('a:', grid.data, len(grid.data))
    sous_listes = []
    liste_originale = grid.data
    for i in range(0, len(liste_originale), grid.wid):
        sous_liste = liste_originale[i:i+ grid.wid]
        sous_listes.append(sous_liste)
    print(sous_listes)
    print(grid.wanted_words)

    print(f"wid: {grid.wid}, hgt: {grid.hgt}")

    if grid is None:
        print ("Failed to create a wordsearch puzzle")
    else:
        print("Génération du mot mélé...")
    ...
    return grid.wanted_words, sous_listes
    #grid.to_pdf("ws.pdf")
    # exit(0)
